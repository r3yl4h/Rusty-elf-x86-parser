...
use crate::header_elf::ElfHeader;
use crate::symtab;
...
fn print_h(sh_table: &[SectionHeader], sh_str: &[u8], file: &mut File) {
    println!("======================================================================================================================================");
...
fn get_flag_description(flag_value: u64) -> &'static str {
    match flag_value {
...
fn get_type_description(types: u32) -> &'static str {
    match types {
        0x0 => "SHT_NULL",
        0x1 => "SHT_PROGBITS",
...
fn get_section_name(offset: usize, sh_str: &[u8]) -> String {
    sh_str[offset..]
        .iter()
        .take_while(|&&c| c != 0)
        .map(|&c| c as char)
...
pub fn pars_section(file: &mut File, elf_header: ElfHeader) -> Result<(), io::Error>{
    let mut section_header = vec![SectionHeader::default(); elf_header.sh_num as usize];
    file.seek(SeekFrom::Start(elf_header.sh_offset))?;
    for i in 0..elf_header.sh_num {
        file.read_exact(unsafe {
            std::slice::from_raw_parts_mut(&mut section_header[i as usize] as *mut SectionHeader as *mut u8, mem::size_of::<SectionHeader>())
        })?;
    }
