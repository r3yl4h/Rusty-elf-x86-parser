...
use std::mem;
use crate::x64::section_elf::SectionHeader64;
...
pub fn parse_symb(file: &mut File, elf_header: Vec<SectionHeader64>) -> Result<(), Error> {
    ...
    let mut i = 0;
    while i * mem::size_of::<Elf64Sym>() < dynsym_sz {
        let mut sym = Elf64Sym::default();
        let absoffset = dynsym_off + i * mem::size_of::<Elf64Sym>();
        file.seek(SeekFrom::Start(absoffset as u64))?;
        file.read_exact(unsafe { std::slice::from_raw_parts_mut(&mut sym as *mut Elf64Sym as *mut u8, mem::size_of::<Elf64Sym>())})?;
        println!("\n-----------------------------------");
        println!(" SYH: {i}");
